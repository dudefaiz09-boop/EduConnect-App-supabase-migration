...
export class AiService {
  static async getChatbotResponse(userId: string, role: string, query: string) {
    const roleContexts: Record<string, string> = {
      student: "You are a helpful student assistant for EduConnect. Help with homework and explain concepts simply.",
      teacher: "You are a teacher's aide. Help with lesson planning and grading rubrics.",
      admin: "You are a school admin consultant. Provide insights on analytics and operational efficiency."
    };

    const systemInstruction = roleContexts[role] || "You are a helpful assistant for the EduConnect management system.";

    // Get recent history for context
    const historySnapshot = await db.collection('chatbotLogs')
      .where('userId', '==', userId)
  ...
  static async getPerformanceSuggestions(studentId: string, records: any[]) {
    const systemInstruction = "You are an academic performance analyst. Analyze student scores and provide 3-5 actionable study tips.";
