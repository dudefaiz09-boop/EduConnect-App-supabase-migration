...
function canViewStudentAssignments(user: NonNullable<Express.Request['user']>, studentId: string) {
  return studentId === user.uid || user.isAdmin || user.permissions?.manageAssignments || user.permissions?.viewReports || (user.permissions?.viewOwnRecords && user.linkedStudentIds?.includes(studentId));
}
...
export class AssignmentsController {
  ...
  static async getSubmissions(req: Request, res: Response, next: NextFunction) {
    try {
      const submissions = await AssignmentsRepository.getSubmissions(req.params.assignmentId, req.tenantId!);
  ...
  static async create(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await AssignmentsRepository.create(req.body, { uid: req.user!.uid, email: req.user!.email, schoolId: req.user!.schoolId }, req.tenantId!);
      res.json(result);
    } catch (error) { next(error); }
  }
  ...
  static async submit(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await AssignmentsRepository.submit(id, req.body, { uid: req.user!.uid, email: req.user!.email, schoolId: req.user!.schoolId }, req.tenantId!);
      res.json(result);
    } catch (error) { next(error); }
  }
  ...
  static async recheck(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await AssignmentsRepository.recheck(req.body, { uid: req.user!.uid, email: req.user!.email, schoolId: req.user!.schoolId }, req.tenantId!);
      res.json({ success: true });
    } catch (error) { next(error); }
  }
}
