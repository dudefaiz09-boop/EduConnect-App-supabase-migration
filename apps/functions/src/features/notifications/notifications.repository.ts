...
type NotificationDoc = { id: string; archivedBy?: string[]; readBy?: string[]; targetUserIds?: string[]; targetRoles?: string[]; targetClasses?: string[]; schoolId?: string; tenantId?: string; archived?: boolean; actorId?: string; title?: string; message?: string; type?: string; href?: string; metadata?: Record<string, unknown>; createdAt?: string; updatedAt?: string };
...
function canSeeNotification(notification: NotificationDoc, user: any, activeTenantId?: string) {
  ...
  const roleMatch = targetRoles.includes('all') || userRoles.some((r: string) => targetRoles.includes(r));
  const classMatch = targetClasses.includes('all') || userClassIds.some((c: string) => targetClasses.includes(c));
...
function toNotificationDoc(doc: any): NotificationDoc { return { id: doc.id, ...(doc.data() as Omit<NotificationDoc, 'id'>) }; }

async function getVisibleNotifications(req: Request) {
  const snapshot = await db.collection('notifications').where('tenantId', '==', req.tenantId).orderBy('createdAt', 'desc').limit(100).get();
  return snapshot.docs.map(toNotificationDoc).filter((n) => canSeeNotification(n, req.user!, req.tenantId));
}
...
export class NotificationsRepository {
  ...
  static async create(data: any, actor: any, tenantId: string) {
    return createNotification({
      title: data.title, message: data.message, type: data.type, href: data.href,
      targetUserIds: data.targetUserIds, targetRoles: data.targetRoles, targetClasses: data.targetClasses,
      schoolId: tenantId, tenantId, actorId: actor.uid, metadata: data.metadata,
    });
  }
  ...
  static async markAllRead(req: Request) {
    ...
    await Promise.all(visible.map((n) => db.collection('notifications').doc(n.id).update({ readBy: Array.from(new Set([...(n.readBy || []), req.user!.uid])), updatedAt: now })));
  ...
  static async markRead(id: string, req: Request) {
    ...
    if (!canSeeNotification(notification, req.user!, req.tenantId)) throw new AppError('Forbidden', 403);
  ...
  static async archiveRead(req: Request) {
    ...
    const now = new Date().toISOString();
    await Promise.all(readNotifications.map((n) => db.collection('notifications').doc(n.id).update({ archivedBy: Array.from(new Set([...(n.archivedBy || []), req.user!.uid])), updatedAt: now })));
  ...
  static async archive(id: string, req: Request) {
    ...
    if (req.user!.isAdmin || notification.actorId === req.user!.uid) {
      ...
      await ref.update({ archivedBy: Array.from(new Set([...(notification.archivedBy || []), req.user!.uid])), updatedAt: new Date().toISOString() });
    }
  }
}
