...
import { createNotificationSchema, notificationIdParamsSchema } from './notifications.validation.js';
import { requireAnyRole } from '../../middleware/permissions.js';

const router: Router = Router();

router.get('/', NotificationsController.list);
router.post('/', requireAnyRole(['admin', 'principal', 'super_admin']), validate(createNotificationSchema), NotificationsController.create);
