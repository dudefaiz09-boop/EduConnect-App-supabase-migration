...
function getProfileTenantId(profile: Record<string, unknown>) { return String(profile.tenantId || profile.schoolId || ''); }

export class UsersRepository {
  static async list(query: { tenantId?: string; role?: string; status?: string; search?: string; limit?: number }, req: Request) {
    ...
    if (query.role) users = users.filter((p) => { const roles = Array.isArray(p.roles) ? p.roles : []; return p.role === query.role || roles.includes(query.role); });
    if (query.status) users = users.filter((p) => p.status === query.status);
    if (query.search) { const q = query.search.toLowerCase(); users = users.filter((p) => `${p.displayName || ''} ${p.email || ''} ${p.role || ''}`.toLowerCase().includes(q)); }
    return users.slice(0, query.limit || 100);
  }

  static async listTenants(req: Request) {
    const allowedTenantIds = req.user!.isSuperAdmin ? req.user!.managedTenantIds || [] : req.tenantId ? [req.tenantId] : [];
    if (allowedTenantIds.length === 0) return [];
    const snapshot = await db.collection('schools').get();
    return snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })).filter((t) => allowedTenantIds.includes(String(t.id)));
  }
  ...
  static async getAuditLogs(query: { targetUid?: string; limit?: number }, tenantId: string) {
    ...
    logs = logs.sort((a, b) => new Date(String(a.timestamp || a.createdAt || 0)).getTime() - new Date(String(b.timestamp || b.createdAt || 0)).getTime());
    return logs.slice(0, query.limit || 100);
  }

  static async updateOwnProfile(uid: string, data: { displayName?: string; photoURL?: string }, email?: string) {
    ...
    if (snapshot.exists) {
      ...
      const after = { ...before, ...(data.displayName ? { displayName: data.displayName } : {}), ...(data.photoURL ? { photoURL: data.photoURL } : {}), updatedAt: new Date().toISOString() };
      await userRef.update(after);
      await supabaseAdmin.from('profiles').upsert({ id: uid, display_name: after.displayName, updated_at: after.updatedAt });
    }
  }
  ...
  static async bulkImport(users: any[], req: Request, actor: Actor) {
    ...
    for (const user of users) {
      try {
        ...
        results.push({ success: false, email: user.email, error: error instanceof Error ? error.message : String(error) });
      }
    }
    return results;
  }
  ...
  static async deleteRequest(uid: string, actor: Actor) {
    ...
    await writeAuditLog({ action: 'user_delete_requested', targetUid: uid, performedBy: actor.uid, details: `Delete requested; ${profile.email || uid} was deactivated instead`, before: profile, after: profile, schoolId: profile.schoolId });
    return profile;
  }
}
