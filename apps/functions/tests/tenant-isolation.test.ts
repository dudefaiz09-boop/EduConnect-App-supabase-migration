...
describe("backend tenant isolation policies", () => {
  it("covers user management tenant rules", () => {
    ...
    expect(() =>
      assertUserCanManageTenant(admin, "tenant-a", "tenant-a"),
    ).not.toThrow();
    expect(() =>
      assertUserCanManageTenant(admin, "tenant-a", "tenant-b"),
    ).toThrow();
    expect(deactivationUpdate("2026-01-01T00:00:00.000Z").status).toBe(
      "inactive",
    );
  });
  ...
  it("covers super-admin managed tenant rules", () => {
    ...
    expect(() =>
      assertUserCanManageTenant(superAdmin, "tenant-a", "tenant-b"),
    ).not.toThrow();
    expect(() =>
      assertUserCanManageTenant(superAdmin, "tenant-a", "tenant-c"),
    ).toThrow();
  });
  ...
  it("covers fee tenant and role rules", () => {
    ...
    expect(isTenantScopedRecord({ tenantId: "tenant-a" }, "tenant-a")).toBe(
      true,
    );
    expect(isTenantScopedRecord({ schoolId: "tenant-a" }, "tenant-a")).toBe(
      true,
    );
    expect(isTenantScopedRecord({ tenantId: "tenant-b" }, "tenant-a")).toBe(
      false,
    );
