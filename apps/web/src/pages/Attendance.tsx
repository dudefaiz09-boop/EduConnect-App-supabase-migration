...
export const AttendancePage = () => {
  ...
  const stats = [
    {
      name: 'Present',
      value: Object.values(dailyRecords).filter((v) => v === 'present').length,
      color: '#10b981',
    },
    {
      name: 'Absent',
      value: Object.values(dailyRecords).filter((v) => v === 'absent').length,
      color: '#ef4444',
    },
    {
      name: 'Late',
      value: Object.values(dailyRecords).filter((v) => v === 'late').length,
      color: '#f59e0b',
    },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            {isStudent
              ? 'View your attendance record and history.'
              : 'Manage daily attendance and view performance reports.'}
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm font-bold text-blue-600 bg-blue-50 px-4 py-2 rounded-xl">
                  Overall:{' '}
                  {Math.round(
                    (history.filter((r) => r.status === 'present').length / (history.length || 1)) *
                      100
                  )}
                  %
                </div>
