...
async function verifyFirestoreRules() {
  ...
  console.log(
    '2. Rules use `request.auth.uid == resource.data.studentId` for record-level protection.'
  );
  console.log(
    '3. Administrative roles are checked via `request.auth.token.roles` and `request.auth.token.permissions`.'
  );

  console.log('\n⚠️ Production Warning:');
  console.log(
    '- Ensure that the Firebase Storage rules are also configured if you are using file uploads.'
  );
  console.log(
    '- Verify that Custom Claims are correctly assigned during user creation in `server.ts`.'
  );
}
