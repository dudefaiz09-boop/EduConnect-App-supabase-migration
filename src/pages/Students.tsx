    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1 text-lg">Manage profiles, academic records, and access.</p>
        </div>
            ...
            >
              <div className="absolute top-0 right-0 p-6">
                <button 
                  onClick={() => {
                    ...
                    setFormData({ email: s.email, password: '', displayName: s.displayName, classId: s.classId || '', section: s.section || '' });
                  }}
              ...
              <div className="flex items-center gap-5 mb-8">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-xl font-black text-slate-900 truncate leading-tight">{s.displayName || 'Unnamed Student'}</h3>
                  <p className="text-sm text-slate-400 font-bold truncate tracking-wide">{s.email}</p>
                </div>
              </div>
      ...
      <AnimatePresence>
        {(isAddModalOpen || selectedStudent) && (
          <div className="fixed inset-0 flex items-center justify-center p-6 z-[100]">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => { setIsAddModalOpen(false); setSelectedStudent(null); }} className="absolute inset-0 bg-black/40 backdrop-blur-md" />
            <motion.div
            ...
            >
              <div className="flex justify-between items-start mb-10">
                <div>
                  <h3 className="text-3xl font-black text-slate-900">{selectedStudent ? 'Edit Profile' : 'New Student'}</h3>
                  <p className="text-slate-500 font-medium mt-1">Configure student identity and academic assignment.</p>
                </div>
                <button onClick={() => { setIsAddModalOpen(false); setSelectedStudent(null); }} className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-600 transition-all">
                  <X size={24} />
                </button>
              </div>

              <form onSubmit={selectedStudent ? handleUpdateStudent : handleCreateStudent} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {!selectedStudent && (
                    <>
                      <div className="space-y-2">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 px-1">Email Address</label>
                        <input 
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 px-1">Temp Password</label>
                        <input 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 px-1">Class</label>
                    <input 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 px-1">Section</label>
                    <input 
                ...
                <div className="flex gap-4 pt-4">
                  <button type="submit" className="flex-1 bg-blue-600 text-white py-6 rounded-[24px] font-black uppercase tracking-widest text-sm shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all flex items-center justify-center gap-3">
                    <Save size={20} />
                    {selectedStudent ? 'Save Changes' : 'Create Student'}
                  </button>
                  <button type="button" onClick={() => { setIsAddModalOpen(false); setSelectedStudent(null); }} className="px-10 bg-slate-100 text-slate-500 py-6 rounded-[24px] font-black uppercase tracking-widest text-sm hover:bg-slate-200 transition-all">Cancel</button>
                </div>
              </form>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-6 z-[100]">
            ...
            >
              <div className="space-y-2 mb-10">
                ...
                <p className="text-slate-500 font-medium">Paste CSV records to create multiple accounts at once.</p>
              </div>
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                   ...
                   <div className="p-6 bg-blue-50 rounded-3xl border border-blue-100">
                     ...
                     <p className="text-sm text-blue-800 font-medium">Email, Full Name, Class, Section, Password (one per line)</p>
                   </div>
                </div>

                <div className="flex gap-4">
                  <button onClick={handleBulkImport} className="flex-1 bg-blue-600 text-white py-6 rounded-[24px] font-black uppercase tracking-widest text-sm shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all">Start Import</button>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-6 z-[100]">
            ...
            >
              <div className="p-10 border-b border-slate-50 flex justify-between items-center">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      ...
                      <p className="text-slate-500 font-medium">Tracking system changes and student lifecycle.</p>
                   </div>
                </div>
                <button onClick={() => setIsAuditModalOpen(false)} className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-600 transition-all">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-4">
                {auditLogs.map((log) => (
                  <div key={log.id} className="p-6 rounded-3xl bg-slate-50 border border-slate-100 flex gap-6 items-start">
                    <div className={cn(
                ...
                {auditLogs.length === 0 && (
                  <div className="text-center py-20">
                    ...
                    <p className="text-slate-400 font-bold">No activity logs found for this selection.</p>
                  </div>
                )}
              </div>
            </motion.div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-32 space-y-4">
          ...
          <p className="text-slate-400 font-black uppercase tracking-widest text-xs">Syncing Registry...</p>
        </div>
      )}
    </div>
  );
